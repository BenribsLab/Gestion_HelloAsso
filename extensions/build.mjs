  ...
  if (manifest.entrypoints?.server) {
    ...
    await build({
      ...
      build: {
        ...
        rollupOptions: {
          // isBuiltin() reconnaît aussi bien "node:stream" que le nom nu historique "stream" —
          // un simple startsWith("node:") laissait passer les paquets (ex. "ws") qui importent
          // encore les modules natifs sans préfixe, silencieusement transformés en stubs
          // navigateur vides par l'heuristique par défaut de Vite ("Class extends value
          // undefined" au chargement).
